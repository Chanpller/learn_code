        ...
        for(Method method : declaredMethods){
            if(action.equals(method.getName())){
                ...
                try {
                    ...
                    if(returnValue==null || "".equals(returnValue.toString())){
                        return;
                    }
                    if(returnValue.toString().startsWith("redirect:")){
                        String redirectStr = returnValue.toString().substring("redirect:".length());
                        resp.sendRedirect(redirectStr);
                    }else if(returnValue.toString().startsWith("json:")){
                        String redirectStr = returnValue.toString().substring("json:".length());
                        resp.setContentType("application/json;charset=utf-8");
                        resp.getWriter().write(redirectStr);
                    }else {
